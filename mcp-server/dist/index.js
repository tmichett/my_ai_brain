...
async function getEmbedding(text) {
    ...
    if (!response.ok) {
        ...
        throw new Error(`Ollama embedding failed (${response.status}): ${msg}. Is Ollama running with model '${OLLAMA_EMBED_MODEL}' pulled?`);
    }
...
server.tool("capture_thought", "Save a new thought to the Open Brain. Generates a vector embedding and stores it for semantic retrieval. Pass metadata (topics, people, type, action_items) if available - the AI calling this tool should extract metadata from the content naturally.", {
    content: z
        .string()
        .describe("The thought to capture - a clear, standalone statement that will make sense when retrieved later"),
    topics: z
        .array(z.string())
        .optional()
...
server.tool("search_thoughts", "Search captured thoughts by meaning using vector similarity. Use when looking for thoughts about a topic, person, or idea previously captured.", {
...
server.tool("list_thoughts", "List recently captured thoughts with optional filters by type, topic, person, or time range.", {
    limit: z.number().optional().default(10).describe("Max results"),
    type: z
        .string()
        .optional()
        .describe("Filter by type: observation, task, idea, reference, person_note, decision, learning"),
    ...
    try {
        ...
        if (person)
            query = query.contains("metadata", { people: [person] });
        if (days) {
...
server.tool("thought_stats", "Get a summary of all captured thoughts: totals, types, top topics, and people mentioned.", {}, async () => {
    try {
        ...
        if (Object.keys(topics).length) {
            ...
            for (const [k, v] of sort(topics))
                lines.push(`  ${k}: ${v}`);
        }
        if (Object.keys(people).length) {
            lines.push("", "People mentioned:");
            for (const [k, v] of sort(people))
                lines.push(`  ${k}: ${v}`);
        }
