#!/usr/bin/env bash
# Travis-Fedora: ensure Open Brain + Agentic OS Podman containers are running.
# Native Podman (no machine). Do not run this on Travis-Mac_Apple.
set -euo pipefail
...
LOG_DIR="${LOG_DIR:-${HOME}/.local/state/ai-brain}"
LOG_FILE="${LOG_DIR}/ensure-ai-brain-services-travis-fedora.log"
mkdir -p "$LOG_DIR"
...
Travis-Fedora: ensure Ollama, Supabase, and agentic-os-dashboard containers are up.
...
discover_kong() {
  podman ps --filter "name=supabase_kong" --format '{{.Names}}' 2>/dev/null | head -1
}
...
resolve_dashboard_script() {
  ...
  if [[ -z "$script" && -n "${AGENTIC_OS_REPO:-}" ]]; then
    script="${AGENTIC_OS_REPO}/run-container-travis-fedora.sh"
  fi
...
services_healthy() {
  local supabase_key secret kong
  container_running ollama || return 1
  kong="$(discover_kong)"
  container_running "$kong" || return 1
  container_running agentic-os-dashboard || return 1
...
start_container_if_needed() {
  ...
  fail "${name} container missing — first-time: see docs/hosts/TRAVIS-FEDORA.md (podman run ollama / supabase start)"
  return 1
}
...
start_supabase_containers() {
  ...
  elif podman ps -a --filter "name=supabase_" --format "{{.Names}}" | grep -q .; then
    log "  supabase: already running"
  else
    fail "no supabase_* containers — first-time: supabase start (docs/hosts/TRAVIS-FEDORA.md)"
    return 1
  fi
}
...
start_agentic_os_dashboard() {
  ...
  fail "agentic-os-dashboard missing and run-container-travis-fedora.sh not found"
  return 1
}

kickstart_user_units() {
  if command -v systemctl >/dev/null 2>&1; then
    systemctl --user start agentic-os-dashboard.service >/dev/null 2>&1 || true
    systemctl --user start agentic-os-agent-runner.service >/dev/null 2>&1 || true
  fi
}

brain_sync_pull_quiet() {
  local sync="${SCRIPT_DIR}/brain-sync.sh"
  [[ -x "$sync" ]] || return 0
  if [[ -z "${VAULT_DIR:-}" && -z "${OBSIDIAN_VAULT_DIR:-}" ]]; then
    warn "VAULT_DIR unset — skip brain-sync pull (run install-ai-brain.sh --vault …)"
    return 0
  fi
  if [[ "$HOOK_MODE" -eq 1 ]]; then
    BRAIN_SYNC_HOOK=1 "$sync" --hook pull >>"$LOG_FILE" 2>&1 || true
  else
    "$sync" pull >&2 || true
  fi
}
...
start_all_services() {
  local failures=0
  log "Ensuring AI Brain + Agentic OS containers (Travis-Fedora)..."
